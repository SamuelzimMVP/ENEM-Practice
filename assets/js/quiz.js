...
function openQuestionDetails(questionId, detail, index) {
  ...
  (q.alternativas || []).forEach(alt => {
    ...
    altsHtml += `
      <div class="detail-alt ${stateClass}">
        <span class="alt-letter">${escapeHtml(alt.letra)}</span>
        ${alt.imgUrl && isImageUrlAllowed(alt.imgUrl)
          ? `<img src="${escapeHtml(alt.imgUrl)}" style="max-width:100%;max-height:120px;object-fit:contain;">`
          : `<span>${escapeHtml(alt.texto)}</span>`
        }
      </div>
    `;
  ...
  const extras = (q.imagens || []).filter(u => !shown.has(u) && isImageUrlAllowed(u));
  ...
  body.innerHTML = `
    ...
    <div class="modal-images" style="margin-bottom:16px;">
      ${uniqueExtrasModal.map(url => `<img src="${escapeHtml(url)}" class="question-main-img" style="margin-bottom:8px;">`).join('')}
    </div>
